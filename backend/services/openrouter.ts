import { MODEL, systemPrompt, roomSystemPrompt } from '../config/ai';
...
export async function generateRoomDateIdeasWithAI(events: any[], coupleContext: any) {
  if (!OPENROUTER_KEY) throw new Error('Missing OpenRouter API key');

  const messages = [
    { role: 'system', content: roomSystemPrompt },
    { role: 'user', content: JSON.stringify({ events, coupleContext }) },
  ];
  ...
  // AI should return JSON only (as required by roomSystemPrompt)
  try {
    const parsed = JSON.parse(text);
    return parsed;
  } catch (err: unknown) {
